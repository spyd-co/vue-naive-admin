  ...
  return {
    ...
    fillForm: (data, selector, cfg) => {
      ...
      $(selector).each((idx, elm) => {
        ...
        for (var k in data) {
          let displayContent =
            spydTool.isObject(data[k]) || Array.isArray(data[k]) ? JSON.stringify(data[k], null, 4) : data[k]
          if (f === k) {
    ...
    fillTree: (nodeList, $container, nodeTpl, selected, root) => {
      ...
      $rootContainer.find('.node-info').each((idx, elm) => {
        if (selected === $(elm)[0].dataset.id) {
          $(elm).find('.node-title').addClass('text-warning').addClass('fw-boldest')
        }
      })
