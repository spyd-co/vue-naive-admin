...
function getSpydApi() {
            ...
            if (resp.error === 5 || resp.error === 5001) {
                ...
                if (spydTool.inIframe()) {
                    if (parent) {
                        ...
                        if (spydIframe) {
                            spydIframe.postEvent("token_expired", resp,
                                { postToTop: true });
                        }
                    }
                }
    ...
    return {
        ...
        register: async function (id, pass, code, dataHandler, errorHandler) {
            ...
            const resp = await this.fetch("v1/user/register/code", "post",
            {
                login_id: id,
                pass: pass,
                code: code
        ...
        auth: async function (id, pass, dataHandler, errorHandler) {
            ...
            const resp = await this.fetch("v1/user/login", "post",
            {
                id: id,
                pass: pass,
                device: ""
        ...
        fetch: async function (api, action, params, errorHandler) {
            ...
            const rawResp = await fetch(
                `${_address}?api=${req["api"]}&act=${req["action"]}`,
                {
                    method: 'POST',
                    headers: {
